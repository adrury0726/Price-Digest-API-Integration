...
{
    [Route("PostTransmissions")]
    [ApiController]
    public class GetTransmissionsController : ControllerBase
    {
        [HttpPost(Name = "PostTransmissions")]
        public IActionResult Post([FromServices] IAPICalls apiCalls, [FromServices] ILogging callLogging, [FromServices] IDBConnection dBConnection, AppBody appBody)
        {
            ...
            try
            {
                ...
                userNum = 0;
                string callType = "GetTransmissionsController Class"; //Used to let devs know exactly where errors are occurring.
                ...
                //Endpoint we're using here
                string endpoint = "values/options?sizeClassId=" + components.SizeClassID + "&year=" + components.ModelYear + "&optionFamily=Transmission";
