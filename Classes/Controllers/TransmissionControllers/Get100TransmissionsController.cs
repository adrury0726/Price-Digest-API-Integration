...
{
    [Route("Post100Transmissions")]
    [ApiController]
    public class Get100TransmissionsController : ControllerBase
    {
        [HttpPost(Name = "Post100Transmissions")]
        public IActionResult Post([FromServices] IAPICalls apiCalls, [FromServices] ILogging callLogging, [FromServices] IDBConnection dBConnection, AppBody appBody)
        {
            ...
            try
            {
                ...
                userNum = 0;
                string callType = "Get100TransmissionsController Class"; //Used to let devs know exactly where errors are occurring.
                ...
                //Endpoint we're using. THe offset 100 means it'll skip the first 100 results and give you the rest. The returns return only up to 100 results.
                string endpoint = "values/options?sizeClassId=" + components.SizeClassID + "&year=" + components.ModelYear + "&optionFamily=Transmission&offset=100";
