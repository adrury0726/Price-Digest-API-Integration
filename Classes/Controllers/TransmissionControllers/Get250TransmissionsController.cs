...
{
    [Route("Post250Transmissions")]
    [ApiController]
    public class Get250TransmissionsController : ControllerBase
    {
        [HttpPost(Name = "Post250Transmissions")]
        public IActionResult Post([FromServices] IAPICalls apiCalls, [FromServices] ILogging callLogging, [FromServices] IDBConnection dBConnection, AppBody appBody)
        {
            ...
            try
            {
                ...
                userNum = 0;
                string callType = "Get250TransmissionsController Class"; //Used to let devs know exactly where errors are occurring.
                ...
                //Endpoint we're using. THe offset 250 means it'll skip the first 250 results and give you the rest. The returns return only up to 250 results.
                string endpoint = "values/options?sizeClassId=" + components.SizeClassID + "&year=" + components.ModelYear + "&optionFamily=Transmission&offset=250";
